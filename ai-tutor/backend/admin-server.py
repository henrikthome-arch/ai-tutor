    ...
    try:
        ...
        try:
            ...
            try:
                ...
                # Store student_subjects_detailed in progress data initially
...
def admin_student_detail(student_id):
    ...
    try:
            ...
            if not show_all_requested and progress.get('student_subjects_detailed'):
                ...
                try:
                    ...
                    if student_grade_int is not None:
                        ...
                        progress['subjects_filtered_count'] = len(all_subjects) - len(grade_relevant_subjects)
                    else:
                        # Student grade is not numeric, show all subjects
                        progress['grade_filter_applied'] = False
        ...
        except Exception as grade_filter_error:
            print(f"❌ DEBUG: Error in grade filtering logic: {grade_filter_error}")
            log_error('ADMIN', f'Error in grade filtering logic: {str(grade_filter_error)}', grade_filter_error, student_id=student_id)
...
def add_school():
   ...
   if request.method == 'POST':
       try:
           # Get curriculum ID from form (can be empty)
           default_curriculum_id = request.form.get('default_curriculum_id')
           if default_curriculum_id and default_curriculum_id.strip():
               default_curriculum_id = int(default_curriculum_id)
           else:
               default_curriculum_id = None
           ...
           new_school = {
               ...
               "core_values": request.form.get('core_values', ''),
               "default_curriculum_id": default_curriculum_id
           }
           
           # Save to database
           saved_school = save_school(new_school)
           if not saved_school:
               # Get curriculums for re-rendering form on error
               curriculums = get_all_curriculums()
               flash('Error saving school to database', 'error')
               return render_template('add_school.html', curriculums=curriculums)
   ...
   # GET request - provide curriculums for dropdown
   try:
       curriculums = get_all_curriculums()
   except Exception as e:
       log_error('DATABASE', 'Error getting curriculums for add school form', e)
       curriculums = []
       flash('Warning: Could not load curriculums for selection', 'warning')
